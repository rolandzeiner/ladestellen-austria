...
`;var Pt,zt;!function(t){t.language="language",t.system="system",t.comma_decimal="comma_decimal",t.decimal_comma="decimal_comma",t.space_comma="space_comma",t.none="none"}(Pt||(Pt={})),function(t){t.language="language",t.system="system",t.am_pm="12",t.twenty_four="24"}(zt||(zt={}));const Ot=(t,e,i,s)=>{s=s||{},i=null==i?{}:i;const r=new Event(e,{bubbles:void 0===s.bubbles||s.bubbles,cancelable:Boolean(s.cancelable),composed:void 0===s.composed||s.composed});return r.detail=i,t.dispatchEvent(r),r},Tt=["Type 2","CCS","CHAdeMO","Type 1","Tesla","Schuko","CEE"];let Mt=class extends ot{constructor(){super(...arguments),this._config={type:"ladestellen-austria-card"}}setConfig(t){this._config={...t}}render(){const t=this._config.connector_types??[];return V`
      <div class="editor">
        <div class="editor-section">
          <div class="section-header">${Et("editor.section_main")}</div>
          ...
          ${this.hass?V`
                ...
                ></ha-selector>
              `:V`<p>${Et("common.loading")}</p>`}

          <ha-textfield
            label=${Et("editor.name")}
        ...
        <div class="editor-section">
          <div class="section-header">${Et("editor.section_display")}</div>
          ...
          ${this.hass?V`
                <ha-selector
                  ...
                  .label=${Et("editor.max_stations")}
                  @value-changed=${this._valueChanged}
                ></ha-selector>
              `:K}

          <div class="toggle-row">
            <label>${Et("editor.show_pricing")}</label>
            <ha-switch
          ...
          <div class="toggle-row">
            <label>${Et("editor.show_amenities")}</label>
            <ha-switch
        ...
        <div class="editor-section">
          <div class="section-header">${Et("editor.section_filters")}</div>

          <div class="toggle-row">
            <label>${Et("editor.only_available")}</label>
            <ha-switch
          ...
          <div class="toggle-row">
            <label>${Et("editor.only_free")}</label>
            <ha-switch
          ...
          <div class="editor-hint">${Et("editor.connector_filter_hint")}</div>
          <div class="chip-row">
            ${Tt.map(e=>V`
                <button
                  type="button"
          ...
          <div class="editor-hint">${Et("editor.hint_compliance")}</div>
        </div>
      </div>
    `}_toggleConnector(t){const e=this._config.connector_types??[],i=e.includes(t)?e.filter(e=>e!==t):[...e,t];this._config={...this._config,connector_types:i},Ot(this,"config-changed",{config:this._config})}_valueChanged(t){if(!this._config||!this.hass)return;const e=t.target;if(!e.configValue)return;const i=void 0!==e.checked?e.checked:t.detail?.value??e.value;this._config[e.configValue]!==i&&(this._config={...this._config,[e.configValue]:i},Ot(this,"config-changed",{config:this._config}))}static{this.styles=St}};t([pt({attribute:!1})],Mt.prototype,"hass",void 0),t([ut()],Mt.prototype,"_config",void 0),Mt=t([ct("ladestellen-austria-card-editor")],Mt),console.info(`%c  Ladestellen Austria Card  %c  ${Et("common.version")} 0.1.0-beta-9  `,"color: white; font-weight: bold; background: #3FA535","color: white; font-weight: bold; background: dimgray"),window.customCards=window.customCards||[],window.customCards.push({type:"ladestellen-austria-card",name:"Ladestellen Austria",description:"Nearby EV charging stations, powered by E-Control Austria",preview:!0,documentationURL:"https://github.com/rolandzeiner/ladestellen-austria"});let Lt=class extends ot{constructor(){super(...arguments),this._expanded=new Set}static getConfigElement(){return document.createElement("ladestellen-austria-card-editor")}static getStubConfig(t,e){const i=e.find(t=>t.startsWith("sensor.")&&t.includes("ladestelle"));return{entity:i??""}}setConfig(t){if(!t)throw new Error(Et("common.invalid_configuration"));this.config={name:"Ladestellen Austria",max_stations:10,show_amenities:!0,show_pricing:!0,only_available:!1,only_free:!1,connector_types:[],...t}}shouldUpdate(t){if(!this.config)return!1;if(t.has("config")||t.has("_expanded"))return!0;const e=t.get("hass");return!e||!this.config.entity||e.states[this.config.entity]!==this.hass.states[this.config.entity]}getCardSize(){const t=this.config?.max_stations??10;return Math.min(3+Math.ceil(t/3),10)}render(){if(!this.hass||!this.config)return V`<ha-card
        ><div class="empty-state">${Et("common.loading")}</div></ha-card
      >`;const t=this.config.entity?this.hass.states[this.config.entity]:void 0;if(!t)return V`
        <ha-card>
          ${this._renderHeader()}
          <div class="empty-state">${Et("card.no_entity")}</div>
    ...
    `}_renderHero(t,e,i,s){if(!t)return V`<div class="hero hero--empty">
        <div class="hero-label">${Et("card.no_stations")}</div>
      </div>`;const r=this._formatKm(t.distance),a=this._heroCity(t),n=e?this._formatKm(e.distance):r,o=Et("card.hero_range").replace("{min}",this._formatKm(t.distance)).replace("{max}",n),l=i===s?Et("card.hero_count").replace("{count}",String(i)):Et("card.hero_count_filtered").replace("{filtered}",String(i)).replace("{total}",String(s));return V`
      <div class="hero">
        ...
        <div class="hero-context">
          <div class="hero-context-1">
            ${Et("card.hero_context").replace("{city}",a)}
          </div>
          ...
          <div class="station-text">
            <div class="station-line-1">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                aria-label=${Et("card.open_in_maps")}
                title=${Et("card.open_in_maps")}
                @click=${t=>t.stopPropagation()}
              >
    ...
    `}_metricClass(t,e){return t.endsWith(" kW")?e?"metric metric-kw metric-kw--dc":"metric metric-kw":t===Et("card.gratis")?"metric metric-free":t.includes("€")?"metric metric-price":"metric metric-plug"}_renderStationDetail(t,e,i,s,r,a,n){const o=this._amenityItems(t);return V`
      <div class="detail">
        ${s||!r?V`<div class="detail-section">
              <div class="detail-label">${Et("card.availability")}</div>
              ${this._statusLine(s,r,e,i)}
            </div>`:K}
        ${a&&o.length>0?V`<div class="detail-section">
              <div class="detail-label">
                ${Et("card.amenities_heading")}
              </div>
              <div class="amenities">
                ${o.map(t=>V`
        ...
        <div class="detail-actions">
          ...
          >
            ...
            <span>${Et("card.open_in_maps")}</span>
          </a>
          ${t.website?V`<a
                class="action-btn"
                href=${t.website}
          ...
          ${t.phoneNumber?V`<a
              ...
              >
                ...
                <span>${Et("card.call")}</span>
              </a>`:K}
        </div>
      </div>
    `}_statusLevel(t,e,i,s){return e?t&&0!==s?0===i?"busy":i<s?"partial":"ok":"unknown":"inactive"}_statusAria(t,e,i){return"inactive"===t?Et("card.inactive"):"unknown"===t?Et("card.status_unknown"):`${e} / ${i} ${Et("card.live_suffix")}`}_statusLine(t,e,i,s){const r=this._statusLevel(t,e,i,s);return"inactive"===r?V`<div class="status-row status-${r}">
        <span class="status-dot status-${r}"></span>
        ${Et("card.inactive")}
      </div>`:"unknown"===r?V`<div class="status-row status-${r}">
        <span class="status-dot status-${r}"></span>
        ${Et("card.status_unknown")}
      </div>`:V`<div class="status-row status-${r}">
      <span class="status-dot status-${r}"></span>
      ${Et("card.status_count").replace("{avail}",String(i)).replace("{total}",String(s))}
    </div>`}_toggle(t){const e=new Set(this._expanded);e.has(t)?e.delete(t):e.add(t),this._expanded=e}_onKey(t,e){"Enter"!==t.key&&" "!==t.key||(t.preventDefault(),this._toggle(e))}_priceText(t){if(0===t.length)return"";if(t.some(t=>t.freeOfCharge))return Et("card.gratis");const e=t.filter(t=>!t.freeOfCharge&&t.priceCentKwh>0).map(t=>t.priceCentKwh);if(e.length>0)return`${this._formatEuro(Math.min(...e))} €/kWh`;const i=t.filter(t=>!t.freeOfCharge&&t.priceCentMin>0).map(t=>t.priceCentMin);return i.length>0?`${this._formatEuro(Math.min(...i))} €/min`:""}_formatEuro(t){const e=t/100;try{return new Intl.NumberFormat("de-AT",{minimumFractionDigits:2,maximumFractionDigits:2}).format(e)}catch{return e.toFixed(2)}}_address(t){const e=[];t.street&&e.push(t.street);const i=[t.postCode,t.city].filter(Boolean).join(" ");return i&&e.push(i),e.join(", ")}_amenityItems(t){return[{flag:t.greenEnergy,icon:"mdi:leaf",label:Et("amenities.green_energy")},{flag:t.freeParking,icon:"mdi:parking",label:Et("amenities.free_parking")},{flag:t.roofedParking,icon:"mdi:home-roof",label:Et("amenities.roofed_parking")},{flag:t.cateringService,icon:"mdi:silverware-fork-knife",label:Et("amenities.catering")},{flag:t.bathroomsAvailable,icon:"mdi:toilet",label:Et("amenities.bathrooms")},{flag:t.restingFacilities,icon:"mdi:sofa",label:Et("amenities.resting")}].filter(t=>t.flag)}_renderFooter(t){const e=t&&t.includes("E-Control")?t:"Datenquelle: E-Control";return V`<div class="footer">${e}</div>`}_formatKm(t){const e="number"==typeof t?t:parseFloat(String(t??""));if(!Number.isFinite(e))return"–";try{return new Intl.NumberFormat("de-AT",{minimumFractionDigits:2,maximumFractionDigits:2}).format(e)}catch{return e.toFixed(2)}}_shortConnector(t,e){switch(t){case"TYPE_2_AC":return"Type 2";case"COMBO2_CCS_DC":return"CCS";case"CHADEMO":return"CHAdeMO";case"TYPE_1_AC":return"Type 1";case"TESLA_S":case"TESLA_R":return"Tesla";case"OTHER":return"DOMESTIC_F"===e?"Schuko":e?.startsWith("CEE")?"CEE":e??"?";default:return t?.replace(/_/g," ")??e??"?"}}static{this.styles=Ct}};t([pt({attribute:!1})],Lt.prototype,"hass",void 0),t([ut()],Lt.prototype,"config",void 0),t([ut()],Lt.prototype,"_expanded",void 0),Lt=t([ct("ladestellen-austria-card")],Lt);export{Lt as LadestellenAustriaCard};
