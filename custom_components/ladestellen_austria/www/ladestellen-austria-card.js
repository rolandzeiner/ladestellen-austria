...
`,Lt=["Type 2","CCS","CHAdeMO","Type 1","Tesla","Schuko","CEE"];let Ut=class extends ot{constructor(){super(...arguments),this._config={type:"ladestellen-austria-card"}}setConfig(t){this._config={...t}}render(){Pt(this.hass?.language);const t=this._config.connector_types??[];return F`
      <div class="editor">
    ...
    `}_renderPinSection(){const t=this._config.entity,e=t?this.hass?.states[t]:void 0,i=e?.attributes?.stations??[],n=this._config.pinned_station_ids??[],a=new Set(n),s=new Set(i.map(t=>t.stationId)),r=n.filter(t=>!s.has(t));return F`
      <div class="editor-section">
        <div class="section-header">${Ot("editor.section_pinned")}</div>
        <div class="editor-hint">${Ot("editor.pin_hint")}</div>

        ${t?0===i.length?F`<div class="editor-hint editor-hint--muted">
              ${Ot("editor.pin_no_stations_yet")}
            </div>`:F`
              <div class="pin-list">
                ${i.map(t=>{const e=a.has(t.stationId),i="number"==typeof t.distance?`${t.distance.toFixed(2)} km`:"";return F`
                    <button
                      type="button"
            ...
            `:F`<div class="editor-hint editor-hint--muted">
    ...
    `}_renderHero(t,e,i,n){if(!t)return F`<div class="hero hero--empty">
        <div class="hero-label">${Ot("card.no_stations")}</div>
      </div>`;const a=this._formatKm(t.distance),s=this._heroCity(t),r=e?this._formatKm(e.distance):a,o=Ot("card.hero_range").replace("{min}",this._formatKm(t.distance)).replace("{max}",r),l=i===n?Ot("card.hero_count").replace("{count}",String(i)):Ot("card.hero_count_filtered").replace("{filtered}",String(i)).replace("{total}",String(n));return F`
      <div class="hero">
        ...
        <div class="station-body">
          ...
          <div class="station-grid">
            <span class="station-metrics">
              ...
              ${o.map(t=>F`<span class="pill plug">${t}</span>`)}
              ${l>0?F`<span class="pill plug plug-more"
                    >+${l}</span
                  >`:K}
            </span>
            ${_&&c?F`<span
    ...
    `}_renderStationDetail(t,e,i,n,a,s,r){const o=this._amenityItems(t),l=n||!a,c=this._address(t);return F`
      <div class="detail">
        ${l?F`<div class="detail-section">
              <div class="detail-label">${Ot("card.availability")}</div>
              ${this._statusLine(n,a,e,i)}
            </div>`:K}
        ${s&&o.length>0?F`<div class="detail-section">
    ...
    `}_statusLevel(t,e,i,n){return e?t&&0!==n?0===i?"busy":i<n?"partial":"ok":"unknown":"inactive"}_statusAria(t,e,i){return"inactive"===t?Ot("card.inactive"):"unknown"===t?Ot("card.status_unknown"):`${e} / ${i} ${Ot("card.live_suffix")}`}_statusLine(t,e,i,n){const a=this._statusLevel(t,e,i,n);return"inactive"===a?F`<div class="status-row status-${a}">
