...
`,Ue=["Type 2","CCS","CHAdeMO","Type 1","Tesla","Schuko","CEE"];let Ie=class extends se{constructor(){super(...arguments),this._config={type:"ladestellen-austria-card"}}setConfig(e){this._config={...e}}render(){Re(this.hass?.language);const e=this._config.connector_types??[];return B`
    ...
    `}_renderHero(e,t,i,n){if(!e)return B`<div class="hero hero--empty">
      ...
      </div>`;const a=this._formatKm(e.distance),r=this._heroCity(e),o=t?this._formatKm(t.distance):a,s=Ne("card.hero_range").replace("{min}",this._formatKm(e.distance)).replace("{max}",o),l=i===n?Ne("card.hero_count").replace("{count}",String(i)):Ne("card.hero_count_filtered").replace("{filtered}",String(i)).replace("{total}",String(n));return B`
      <div class="hero">
    ...
    `}_renderRackSlot(e){const t=this._pointPowerType(e),i=this._rackSlotStatus(e.status),n=this._pointTooltip(e),a=t?B`<span class="power-badge" data-type=${t}
          >${t.toUpperCase()}</span
