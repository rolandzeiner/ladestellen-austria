...
`;var Ot,Tt;!function(t){t.language="language",t.system="system",t.comma_decimal="comma_decimal",t.decimal_comma="decimal_comma",t.space_comma="space_comma",t.none="none"}(Ot||(Ot={})),function(t){t.language="language",t.system="system",t.am_pm="12",t.twenty_four="24"}(Tt||(Tt={}));const Mt=(t,e,i,s)=>{s=s||{},i=null==i?{}:i;const a=new Event(e,{bubbles:void 0===s.bubbles||s.bubbles,cancelable:Boolean(s.cancelable),composed:void 0===s.composed||s.composed});return a.detail=i,t.dispatchEvent(a),a},Lt=["Type 2","CCS","CHAdeMO","Type 1","Tesla","Schuko","CEE"];let Ut=class extends ot{constructor(){super(...arguments),this._config={type:"ladestellen-austria-card"}}setConfig(t){this._config={...t}}render(){Ct(this.hass?.language);const t=this._config.connector_types??[];return F`
      <div class="editor">
    ...
    `}_toggleConnector(t){const e=this._config.connector_types??[],i=e.includes(t)?e.filter(e=>e!==t):[...e,t];this._config={...this._config,connector_types:i},Mt(this,"config-changed",{config:this._config})}_valueChanged(t){if(!this._config||!this.hass)return;const e=t.target;if(!e.configValue)return;const i=void 0!==e.checked?e.checked:t.detail?.value??e.value;this._config[e.configValue]!==i&&(this._config={...this._config,[e.configValue]:i},Mt(this,"config-changed",{config:this._config}))}static{this.styles=Pt}};t([pt({attribute:!1})],Ut.prototype,"hass",void 0),t([ut()],Ut.prototype,"_config",void 0),Ut=t([ct("ladestellen-austria-card-editor")],Ut),console.info(`%c  Ladestellen Austria Card  %c  ${St("common.version")} 0.1.0-beta-15  `,"color: white; font-weight: bold; background: #3FA535","color: white; font-weight: bold; background: dimgray"),window.customCards=window.customCards||[],window.customCards.push({type:"ladestellen-austria-card",name:"Ladestellen Austria",description:"Nearby EV charging stations, powered by E-Control Austria",preview:!0,documentationURL:"https://github.com/rolandzeiner/ladestellen-austria"});let Nt=class extends ot{constructor(){super(...arguments),this._expanded=new Set}static getConfigElement(){return document.createElement("ladestellen-austria-card-editor")}static getStubConfig(t,e){const i=e.find(t=>t.startsWith("sensor.")&&t.includes("ladestelle"));return{entity:i??""}}setConfig(t){if(!t)throw new Error(St("common.invalid_configuration"));this.config={name:"Ladestellen Austria",max_stations:10,show_hero:!0,show_amenities:!0,show_pricing:!0,sort_by_power:!1,only_available:!1,only_free:!1,connector_types:[],...t}}shouldUpdate(t){if(!this.config)return!1;if(t.has("config")||t.has("_expanded"))return!0;const e=t.get("hass");return!e||!this.config.entity||e.states[this.config.entity]!==this.hass.states[this.config.entity]}getCardSize(){const t=this.config?.max_stations??10;return Math.min(3+Math.ceil(t/3),10)}render(){if(Ct(this.hass?.language),!this.hass||!this.config)return F`<ha-card
      ...
      `;const e=t.attributes.stations??[],i=!0===t.attributes.live_status_available,s=this._filterStations(e),a=s[0],r=this._sortStations(s),n=Math.max(1,this.config.max_stations??10),o=r.slice(0,n),l=o[o.length-1],c=!1!==this.config.show_hero;return F`
      <ha-card>
        ${this._renderHeader()}
        ${c?this._renderHero(a,l,s.length,e.length):B}
    ...
    `}_sortStations(t){return this.config.sort_by_power?[...t].sort((t,e)=>{const i=Math.max(0,...(t.points??[]).map(t=>t.capacityKw??0)),s=Math.max(0,...(e.points??[]).map(t=>t.capacityKw??0));return s!==i?s-i:(t.distance??1/0)-(e.distance??1/0)}):t}_filterStations(t){const e=this.config.only_available??!1,i=this.config.only_free??!1,s=this.config.connector_types??[];return e||i||0!==s.length?t.filter(t=>{if(e){const e="ACTIVE"===t.stationStatus&&(t.points??[]).some(t=>"AVAILABLE"===t.status);if(!e)return!1}if(i){const e=(t.points??[]).some(t=>t.freeOfCharge);if(!e)return!1}if(s.length>0){const e=new Set((t.points??[]).flatMap(t=>(t.connectorType??[]).map(t=>this._shortConnector(t.consumerName,t.key)))),i=s.some(t=>e.has(t));if(!i)return!1}return!0}):t}_renderHeader(){const t=this.config?.name??"Ladestellen Austria";return F`
      <div class="header">
        <a
          class="brand-link"
    ...
    `}_renderHero(t,e,i,s){if(!t)return F`<div class="hero hero--empty">
        <div class="hero-label">${St("card.no_stations")}</div>
      </div>`;const a=this._formatKm(t.distance),r=this._heroCity(t),n=e?this._formatKm(e.distance):a,o=St("card.hero_range").replace("{min}",this._formatKm(t.distance)).replace("{max}",n),l=i===s?St("card.hero_count").replace("{count}",String(i)):St("card.hero_count_filtered").replace("{filtered}",String(i)).replace("{total}",String(s));return F`
      <div class="hero">
    ...
    `}_heroCity(t){return t.city||t.label||""}_renderStation(t,e){const i=t.points??[],s=i.some(t=>(t.electricityType??[]).includes("DC")),a=i.reduce((t,e)=>Math.max(t,e.capacityKw??0),0),r=Array.from(new Set(i.flatMap(t=>(t.connectorType??[]).map(t=>this._shortConnector(t.consumerName,t.key))))),n=r.length<=2?r.join(", "):`${r.slice(0,2).join(", ")} +${r.length-2}`,o=this._priceText(i),l=i.length,c=i.filter(t=>"AVAILABLE"===t.status).length,d="ACTIVE"===t.stationStatus,h=this._statusLevel(e,d,c,l),p=this._expanded.has(t.stationId),u=`https://www.google.com/maps/search/?api=1&query=${t.location.lat},${t.location.lon}`,f=this.config?.show_amenities??!0,g=this.config?.show_pricing??!0,m=[];return a>0&&m.push(`${a} kW`),n&&m.push(n),g&&o&&m.push(o),F`
      <li
      ...
      >
          ...
          <div class="station-text">
            <div class="station-line-1">
              <span class="station-metrics">
                ${m.map((t,e)=>F`
                    ${e>0?F`<span class="metrics-sep">·</span>`:B}
                    <span
        ...
        ${p?this._renderStationDetail(t,c,l,e,d,f,u):B}
      </li>
    `}_metricClass(t,e){return t.endsWith(" kW")?e?"metric metric-kw metric-kw--dc":"metric metric-kw":t===St("card.gratis")?"metric metric-free":t.includes("€")?"metric metric-price":"metric metric-plug"}_renderStationDetail(t,e,i,s,a,r,n){const o=this._amenityItems(t),l=s||!a,c=this._address(t);return F`
      <div class="detail">
